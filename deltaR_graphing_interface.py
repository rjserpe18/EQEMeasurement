...
class interface(Frame):
    ...
    def initUI(self, sets):
        ...
        for set in sets:
            ...
            dataset_box = Entry(current_frame, fg='#009999', bg = '#C0C0C0', justify = 'center')
            dataset_box.insert(0,'Dataset ' + str(sets.index(set)+1))
            self.data_titles.append(dataset_box)
            dataset_box.pack(expand=True, fill = 'x')
        ...
        self.ax.set_xticks(gf.linspace(self.x_minmin,self.x_maxmax,20))
        self.ax.set_yticks(gf.linspace(self.y_minmin,self.y_maxmax,20))
    ...
    def xlimit(self):
        if self.xlim_lower_box.get() != self.xlow:
            ...
            self.ax.set_xticks(gf.linspace(float(lower_text), self.ax.get_xlim()[1], 20))
        ...
        if self.xlim_higher_box.get() != self.xhigh:
            ...
            self.ax.set_xticks(gf.linspace(self.ax.get_xlim()[0], float(higher_text), 20))
            self.canvas.get_tk_widget().update()
            self.canvas.draw()
    ...
    def ylimit(self):
        if self.ylim_lower_box.get() != self.ylow:
            ...
            self.ax.set_yticks(gf.linspace(float(lower_text), self.ax.get_ylim()[1], 20))
            self.canvas.get_tk_widget().update()
            self.canvas.draw()
        ...
        if self.ylim_higher_box.get() != self.yhigh:
            ...
            self.ax.set_yticks(gf.linspace(self.ax.get_ylim()[0], float(higher_text), 20))
            self.canvas.get_tk_widget().update()
            self.canvas.draw()
